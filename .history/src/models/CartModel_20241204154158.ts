...
export const addItemToCart = async (
  ...
  await pool.query(
      'INSERT INTO CartItems (cart_id, product_id, quantity, price, product_name ) VALUES (?, ?, ?, ?, ?)',
      [cartId, productId, quantity, price, product_name]
  );
};
...
export const getCartItemsByCartId = async (cartId: number): Promise<CartItem[]> => {
    ...
    // Force the result type to CartItem[]
...
export const findCartProduct = async (cart_id: number, productId: number): Promise<number | null> => {
  ...
  if (rows.length > 0) {
    return rows[0].quantity; // Returns the quantity of the product
  }
  return null; // If the product is not found, returns null
};